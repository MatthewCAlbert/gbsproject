...
<head>
    ...
    ?>
    <title>Transfer - GBS eMoney Admin</title>
                                ...
                                <div class="row">
                                    ...
                                    <?php
                                        if(isset($_POST['submit'])){
                                            ...
                                            if( $value < 1000 ){
                                                header('Location: ../transfer?warning=minimum-value-Rp-1.000');
                                                exit();
                                            }
                                            ...
                                            $sql = "SELECT * FROM `student` WHERE `$by`='$receiver'";
                                            $res = $conn->query($sql);
                                            if( $res->num_rows > 0 ){
                                                $status2 = 'student';
                                                $row2 = mysqli_fetch_array($res);
                                            }
                                            $sql = "SELECT * FROM `teacher` WHERE `$by`='$receiver'";
                                            $res = $conn->query($sql);
                                            if( $res->num_rows > 0 ){
                                                $status2 = 'teacher';
                                                $row2 = mysqli_fetch_array($res);
                                            ...
                                            if($status1 != false && $status2 != false){
                                                if( $row1['status'] == 'banned' ){
                                                    header('Location: ../transfer?warning=sender-user-is-banned');
                                                    exit();
                                                }
                                                ...
                                                if( $current_balance_sender >= $value ){
                                                    ...
                                                    $sql = "UPDATE `$status2` SET `balance`='$new_balance_receiver' WHERE `$by`='$receiver'";
                                                    $res2 = $conn->query($sql);
                                                    $sql = "INSERT INTO `balance_history`(`id`,`balance`) VALUES ('".$row2['id']."','$new_balance_receiver')";
                                                    $res22 = $conn->query($sql);

                                                    if( $res1 && $res2 ){
                                                        echo '<p class="alert alert-success">Transfer Success! (Rp '.number_format($value).')( '.$row1['name'].' to '.$row2['name'].' )</p>';
                                                        $desc = 'Transfer Success!';
                                                        $request_res = 'success';
                                                        if( !$res11 || !$res22 ){
                                ...
                                <div class="row">
                                    ...
                                    <div class="container">
                                        <form method="post" action="index.php" id="form">
                                            ...
                                            <label>ID / RFID Card ID (receiver)</label>
