<?php
    ...
    */
    ...
    $log_text = array();
    $api_dir = "http://192.168.100.14/gbs-api/sc/";
    //$api_dir = "https://api.gbsproject.com/sc/";
    $action = false;
    ...
    if($res){
        if($res->num_rows>0){
            while($row=mysqli_fetch_array($res)){
                ...
                $sql2 = "SELECT `description` FROM `transaction` WHERE `description` LIKE '%$id%'";
                $res_des = $conn->query($sql2);
                $not_exist = false;
                if( $res_des ){
                    if( $res_des->num_rows == 0 ){
                        $not_exist = true;
                    }
                }
                    ...
                    if( $user_status != "" ){
                        ...
                        if($res3){
                            if($res3->num_rows>0){
                                ...
                                $desc = 'Top Up via Midtrans Pay ID('.$id.')';
    ...
    sendMessage($log_text,true); //comment this to log instead
